function [hip_cntrl_torque, knee_cntrl_torque, xdes, ydes, phi1des, ...
    phi2des] = traj_controller_2(phi1, phi1dot, phi2, phi2dot, t,...
    x_ell_cntr, y_ell_cntr, a_ell, b_ell, traj_vel, kp, kd, phase)

global BC l1 IBC_angle 

% Trajectory planning
angle = - traj_vel * t  + phase;

% Make it semi-elliptical
param = 0.01;
if abs(mod(angle,2*pi))>pi
    b_ell = param * b_ell;
end

xdes = x_ell_cntr + a_ell * cos(angle);
ydes = y_ell_cntr + b_ell * sin(angle);
